...
import { pcmToBase64, startPCMRecorder, stopMicrophone } from "~/lib/pcmAudio";
...
export default function PCMVoiceRecorder({
  ...
  const startRecording = async () => {
    try {
      ...
      const { recorderNode, audioContext, micStream } = await startPCMRecorder(
        (pcmData: Int16Array) => {
          ...
          if (normalizedLevel > VOICE_THRESHOLD) {
            // Voice detected
            console.log(
              `🎤 Voice detected: level ${normalizedLevel.toFixed(2)} > ${VOICE_THRESHOLD}`,
            );
            ...
            console.log(
              `🔇 Silence detected: ${sinceVoiceMs}ms since voice, ${collectDuration}ms total duration`,
            );

            if (
              sinceVoiceMs >= SILENCE_MS &&
              collectDuration >= MIN_DURATION_MS
            ) {
              console.log(
                `📤 AUTO-SENDING: Silence threshold reached (${sinceVoiceMs}ms >= ${SILENCE_MS}ms)`,
              );
              sendBufferedAudio();
              resetBuffer();
            }
      ...
      console.log(
        `✅ PCM recording started: ${audioContext.sampleRate}Hz → 16kHz (resampled)`,
      );
    } catch (error) {
  ...
  const validateAudioQuality = (audioData: Int16Array): AudioQualityResult => {
    ...
    const avgEnergy = audioData.length > 0 ? totalEnergy / audioData.length : 0;
  ...
  const sendCurrentBuffer = () => {
    ...
    if (!qualityCheck.isValid) {
      ...
      console.log(
        `🗑️ Silently discarding problematic audio: ${qualityCheck.reason}`,
      );
    ...
    const base64Data = btoa(
      String.fromCharCode(...new Uint8Array(combinedAudio.buffer)),
    );
    const transcription = `[Voice message - ${qualityCheck.durationMs}ms]`;
    onAudioData(base64Data, "audio/pcm", transcription);
    console.log(
      `📤 Sent audio segment: ${combinedAudio.length} samples (${qualityCheck.durationMs}ms)`,
    );
  ...
  useEffect(() => {
    console.log(
      `🎛️ PCMVoiceRecorder disabled state changed: ${disabled}, isRecording: ${isRecording}`,
    );
  ...
  return (
    <div className="flex items-center space-x-3">
      ...
      >
        ...
        {isRecording && (
          <div className="absolute inset-0 animate-ping rounded-full border-2 border-red-300" />
        )}
      </button>
      ...
      {isRecording && bufferLength > 0 && (
        <button
          ...
          className="rounded-full bg-green-500 px-3 py-1 text-xs text-white transition-colors duration-200 hover:bg-green-600"
        >
          Send Now ({Math.round(bufferLength / 60)}s){" "}
          {/* Approximate seconds based on 60fps */}
        </button>
      )}
      ...
      {isRecording && (
        <div className="flex items-center space-x-1">
          ...
          <span className="ml-2 text-xs font-medium text-green-400">
            🎙️ Live PCM • {sampleRate}Hz→16kHz{" "}
            {isCollecting && "• 🔊 Voice Detected"}
          </span>
        </div>
      )}
      ...
      {!isRecording && (
        <span className="text-xs text-gray-400">
          {canRecord
            ? "Click for continuous voice recording (auto-sends on pause)"
            : "Voice disabled"}
        </span>
      )}
